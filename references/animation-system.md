...
2. Freeze the approved silhouette, moving assemblies, and provisional state graph.
3. Build rig hierarchy, pivots, sockets, default pose, and root-motion policy.
4. Graybox the highest-risk clips and transitions before texture detail.
5. Author primary poses, timing, interpolation, contacts, clearances, and secondary motion.
6. Add the central event table; link particle, sound, projectile, hitbox, and gameplay contracts.
7. Test loops, every legal transition, interruption, retrigger, cleanup, collision, and LOD.
8. Reopen the saved model and verify in actual Blockbench; then test controllers, events, blending, and root motion in the actual target runtime.
