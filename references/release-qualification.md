...
Use an RC label while any matrix row lacks real-project evidence. Do not describe an RC as production-ready. A formal version may be published only when focused validators, `validate_asset_bundle.py`, `validate_release_evidence.py`, actual Blockbench review, and every claimed verified-support runtime row pass with archived evidence.
