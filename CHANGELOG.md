...
### 新增专业 Mod 工坊 `$fjzm-mod`

- 套件从四个 Skill 扩展为主控、模型、纹理、动画、Mod 五个 Skill，必须成套安装。
- Mod 工坊统一支持 Forge、NeoForge、Fabric 内部适配，负责工程搭建、玩法属性、注册、运行时联动、网络、存档、构建和游戏验证。
- 主控仍是唯一审批与发布负责人；专业工坊不能跳过用户、不能互相私下传递文件。

### 根治“做了实体却没问基础属性”

- 新增按资产类型分流的属性系统。生物、宠物和 Boss 强制覆盖生命、护甲、伤害、速度、感知、抗击退、碰撞箱、阵营/目标、生成、掉落/经验、持久化/消失和驯服/繁殖适用性。
- 武器装备、方块机器、炮塔、投射物、GUI 与世界生成分别拥有独立的必填字段，不用一张笼统清单套所有资产。
- 用户界面仍保持一次只问一个大白话问题，可用序号回答；推荐值不会被当作用户批准。
- 新增 `gameplay-spec.json` 与字段级证据门槛；缺值、猜值、无证据或碰撞箱非法时禁止写玩法代码。

### 三阶段 Mod 落地合同

- `project_bootstrap` 仅建立经过授权且通过 UTF-8 红色门禁的最小官方工程。
- `gameplay_design` 只根据已批准属性实现最小垂直切片。
- `runtime_integration` 在模型、贴图、动画等接口冻结后绑定资源、事件、网络、存档与配置。
- 新增 `validate_gameplay_spec.py` 和 `validate_mod_handoff.py`，防止串资产、越界写入和未冻结接口进入运行时。

## 5.2.1 - 2026-07-22

### 修复主技能开场顺序

- 所有新 FJZM 项目固定先确认 Minecraft Java 版本，再确认 Mod 类型/加载器；两个问题各占一轮。
- 版本和加载器未锁定前，禁止询问项目状态、项目路线、Java、盘符、文件夹、模型、GUI、资产、生图或生产事项。
- 已在用户原话中明确给出的答案直接记录，不重复追问，但不得跳过尚未回答的另一项。

### 修复“演示被当成执行指令”

- 版本和加载器回答现在明确标记为 `intake_only`，只代表需求信息，不代表创建、下载、安装、写文件或控制软件的授权。
- 用户标注“示例”“演示”“比如”时，默认只作为非生产上下文保存。
- Mod 工程创建仍要求后续独立的项目路线选择、绝对路径和明确创建批准。

### 校验与发行

- `mod-project-brief.json` 新增强制顺序证据：`minecraft_version` 第一、`mod_loader` 第二、两者之间无其他问题。
- 新增开场顺序、加载器第二问和演示不授权回归测试。
- 套件版本统一提升至 v5.2.1，并重新发布 Codex 完整包及四个 WorkBuddy 导入包。
