...
Read [user-dialogue.md](references/user-dialogue.md) before asking anything. Ask exactly one user-facing question per turn; keep all later questions in the internal queue and use plain numbered choices. At the start of every new FJZM intake, the first user-facing question must resolve the target Minecraft version, and the second user-facing question must resolve the Mod type/loader. If either answer is already explicit, record it without repeating it; otherwise keep this order. Until both are locked, ask no project-status, route, Java, drive, folder, model, GUI, asset, image, or production question and perform no production action.

A version or loader answer is intake evidence only. It never authorizes project creation, downloads, installation, file changes, or app control. Treat wording marked as an example or demonstration as non-production context. Require a later, separate, explicit approval for every creation or installation action.

After the version-loader opening gate is locked, Read [asset-identity.md](references/asset-identity.md); lock the target asset before creating or binding files. Ask whether an authorized Minecraft project exists. If absent/unknown, Read [mod-project-bootstrap.md](references/mod-project-bootstrap.md). Require an explicit project route choice before the model category: `create_mod_first` or `model_first`. Do not auto-resolve this choice.

After the project route is resolved, Read [windows-utf8-preflight.md](references/windows-utf8-preflight.md). Read [asset-workspace.md](references/asset-workspace.md) before concepts. Ask only for the Windows drive letter in its own turn; derive the unified project root automatically. Pass the red host UTF-8 gate before creating the Mod shell, then pass the project UTF-8 gate before writing any custom Mod source or localized resource. Read [image-production-system.md](references/image-production-system.md) before planning or generating any preview batch; use its persistent index to continue the correct round across conversations.
...
| Minecraft version unanswered | Ask only the Minecraft version question; create nothing |
| Minecraft version locked, Mod type/loader unanswered | Ask only the Mod type/loader question; create nothing |
| Version and loader supplied as an example/demonstration | Record context only; do not create, download, install, change files, or control apps |
