...
$skillNames = @('fjzm', 'fjzm-model', 'fjzm-texture', 'fjzm-animation', 'fjzm-mod')
$sourcePaths = @{}
$targetPaths = @{}
...
$legacyV52 = $existing.Count -eq 4 -and $existing -contains 'fjzm' -and $existing -contains 'fjzm-model' -and $existing -contains 'fjzm-texture' -and $existing -contains 'fjzm-animation'
$completeV53 = $existing.Count -eq 5

if ($existing.Count -ne 0 -and -not ($legacyV3 -or $legacyV4 -or $legacyV52 -or $completeV53)) {
    throw "Refusing partial suite installation. Found: $($existing -join ', '). Expected none, a recognized v3/v4/v5.2 installation, or all five v5.3 skills."
}
if ($existing.Count -gt 0 -and -not $BackupAndReplace) {
    throw 'Existing FJZM installation detected. Re-run with -BackupAndReplace to back it up and install all five v5.3 skills together.'
}
...
try {
    ...
    Write-Host 'Installed FJZM suite 5.3.0:'
    foreach ($name in $skillNames) {
    ...
    Write-Host 'Restart Codex or start a new task before invoking $fjzm and its four specialist skills.'
}
catch {
